							...
							<div id="gallery_01">
							 	@if(count($image_products) > 0)
							 		@foreach($image_products as $image_product)
									 	<a data-image="uploaded/product/{{$image_product->name}}" data-zoom-image="uploaded/product/{{$image_product->name}}">
					...
					<div class="col-12 col-sm-12 col-md-5 col-lg-6 product-content" >
						...
						<div class="slelect-size-box">
								...
								>
									@if(count($product->product_properties) > 0)
										@foreach($product->product_properties as $sizes)
											<option name='size' value="{{$sizes->size_id}}" data-quantity={{$sizes->quantity}}>{{$sizes->size->name}}	
											</option>
										@endforeach
									@else
							...
							<div class='selector-wrapper'>
								...
								<select class="single-option-selector" id="product__select-quantity">
									@for($i = 1; $i<=10 ; $i++)
										<option name='sl' value="{{$i}}">{{$i}}</option>
									@endfor
								</select>
							</div>
						</div>
						...
						<div class="product-submit-cart">
							<span class='a__submit'>
								<a href="JavaScript:void(0)" id="add" data-product-id='{{$product->id}}' class='btn__link btn__link-cart btn__add-to-cart'> Mua hàng <i class="fa fa-shopping-cart" aria-hidden="true"></i></a>
							</span>
